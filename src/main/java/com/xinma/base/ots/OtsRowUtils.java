...
public class OtsRowUtils {
	...
	private static void exceptionCheck(Exception e, int retryCnt) {
		if (retryCnt >= allowRetryCnt) {
			// 重试次数达到重试上限异常
			throw new OtsException(e, OtsError.ReachRetryLimitErr);
		}
		...
		if (OtsExceptionUtils.isRetryException(e)) {
			...
			throw new OtsException(e, OtsError.OTSExceptionErr);
		}
	}
		...
		for (Entry<String, List<RowStatus>> entry : status.entrySet()) {
			...
			for (int index = 0; index < statuses.size(); index++) {
				...
				if (!rowStatus.isSucceed()) {
					logger.error("访问OTS服务时，发生BatchWriteRowErr, OTS服务返回错误号为 " + rowStatus.getError().getCode()
							+ " ，错误信息为 " + rowStatus.getError().getMessage(),
							new OtsException(OtsError.BatchWriteRowErr));
